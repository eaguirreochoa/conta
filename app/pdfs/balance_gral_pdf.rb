class BalanceGralPdf < Prawn::Document
	TIPO = {:ACT => 'ACTIVO', :PAS => 'PASIVO', :PAT => 'PATRIMONIO'}
	...
	def data
		...
		# 		" SELECT #{@sesion}, (11 || c.id) em_of_c, 1 empresa_id, 1 oficina_id, c.id catalogo_id, n.numnivel, c.padre_id, g.naturaleza, 't', case when esbalance = 't' and esestresul = 'f' and g.naturaleza = 'D' then 'ACT' when esbalance = 't' and esestresul = 'f' and g.naturaleza = 'A' then 'PAS' when esbalance = 'f' and esestresul = 't' and g.naturaleza = 'D' then 'GAS' when esbalance = 'f' and esestresul = 't' and g.naturaleza = 'A'  then 'ING' else 'PAT' end tipo, 0,0,0, '#{Time.now.to_date}', '#{Time.now.to_date}'"+ 
		# 		" from catalogos c INNER JOIN grupos g on c.activo = 't' and g.id = c.grupo_id inner join nivels n on n.id = c.nivel_id where c.estransaccional = 't' and (g.esbalance = 't');"
		# ActiveRecord::Base.connection.execute(@sql)

		#inseta el plan de cuentas para ACT PAS PAT
		@sql = "insert into balance_grals (sesion, em_of_c, empresa_id, oficina_id, catalogo_id,  numnivel, padre_id, naturaleza, estransaccional, tipo, saldo, saldob, saldoc, created_at, updated_at)" +
				" SELECT #{@sesion}, (11 || c.id) em_of_c, 1 empresa_id, 1 oficina_id, c.id catalogo_id, n.numnivel, c.padre_id, g.naturaleza, 't', case when g.nombre = 'ACTIVO' then 'ACT' when g.nombre = 'PASIVO' then 'PAS' when g.nombre = 'PATRIMONIO' then 'PAT' end tipo, 0,0,0, '#{Time.now.to_date}', '#{Time.now.to_date}'"+ 
				" from catalogos c INNER JOIN grupos g on c.activo = 't' and g.id = c.grupo_id inner join nivels n on n.id = c.nivel_id where c.estransaccional = 't' and (g.esbalance = 't');"
		ActiveRecord::Base.connection.execute(@sql)

		#actualiza debe, haber de las cuentas transaccionales ACT PAS PAT		
		...
		@sql = "update balance_grals set saldo = case when tipo in ('ACT', 'GAS') then (debe - haber) when tipo in('PAS','ING') then (haber - debe) else 0 end " +
		 	  " where balance_grals.sesion = #{@sesion};"
		ActiveRecord::Base.connection.execute(@sql)

		#actuliza debe(GAS) haber(ING), en cuentas transaccionales solo PAT
		...
		#select empresa_id, oficina_id, tipo, sum(saldo) saldo, sum(saldob) saldob, sum(saldoc) saldoc from  balance_grals where sesion = 844 group by empresa_id, oficina_id, tipo;
		@sql = "insert into balance_grals (codigo, sesion, em_of_c, empresa_id, oficina_id, catalogo_id, tipo, numnivel, padre_id, naturaleza, estransaccional, saldo, saldob, saldoc, created_at, updated_at)" +
	...
	def line_item_rows
		...
		@balance_gral.where(tipo: ['ACT','PAS','PAT'], estransaccional: 'f', sesion: @sesion).map do |det|			

			if det.codigo == '999999'		

				if not (@ciclos[0].blank?)
					
						arr << [["TOTAL", TIPO[det.tipo.to_sym]].join(" "), det.saldo.round(2), det.saldob] 
				else  	
					if not (@ciclos[1].blank?)
						arr << [["TOTAL", TIPO[det.tipo.to_sym]].join(" "), det.saldo.round(2), det.saldob.round(2), det.saldoc.round(2)] 
					else
